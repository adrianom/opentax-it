...
| Importi della dichiarazione in unità di euro (≥ 50 cent per eccesso); rate al centesimo | `tax-computation.ts` → `roundEuro` (ogni rigo LM/RR/RN62), `installment-plan.ts` (centesimi) | Istr. Redditi PF 2026 Fasc. 1 "Modalità di arrotondamento" e §7 "Modalità e termini di versamento" | 2026-09-21 |
...
| Compensazione dei crediti (LM47, RR8) nelle deleghe | Non implementata: i crediti sono mostrati ma non usati; le deleghe contengono solo importi a debito | Modulo compensazioni (art. 17 D.Lgs. 241/97; visto di conformità sopra 5.000 €) |
