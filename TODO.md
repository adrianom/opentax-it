...
Oggi le fonti verificate sono sparse in tre formati: citazioni in prosa in [docs/normativa-2026.md](docs/normativa-2026.md), matrice in [docs/compliance.md](docs/compliance.md), `sourceRefs` nei set di regole. Manca un registro unico, i documenti letti non sono archiviati (se l'ente cambia URL o aggiorna un PDF non resta traccia di cosa abbiamo letto) e nessun controllo automatico verifica le citazioni. Decisione (24/09/2026): **documenti archiviati nel repository**.
- **Registro** `docs/fonti/registro.json`: una voce per documento con id stabile (es. `inps-circ-62-2026`), ente, titolo, tipo (legge, circolare, istruzioni, specifica, pagina web), URL, data di consultazione, impronta SHA-256 e percorso della copia archiviata.
- **Archivio** `docs/fonti/documenti/`: copia dei PDF e delle pagine lette (per le pagine web, testo estratto con data). Base legale: L. 633/1941 art. 5 (Normattiva, testo vigente): "Le disposizioni di questa legge non si applicano ai testi degli atti ufficiali dello stato e delle Amministrazioni pubbliche". **Da valutare** caso per caso i documenti che non sono atti ufficiali in senso stretto (guide divulgative, modelli): il modello F24 oggi non è ridistribuito per scelta (`f24-pdf.service.ts`), da riallineare.
- **Citazioni** collegate al registro: id della fonte, sezione o pagina, testo esatto, regola o funzione a cui si riferisce. I `sourceRefs` dei set di regole passano a usare l'id della fonte; normativa e matrice di conformità generate o controllate da questi dati.
- **Test automatici**: ogni `sourceRef` e ogni riga della matrice puntano a una fonte registrata; ogni citazione compare nel testo del documento archiviato (normalizzando spazi e a capo); l'impronta del file archiviato coincide con quella del registro.
- **Pagina "Fonti" nell'app**: elenco con ricerca per ente e argomento; da ogni regola (imposte, F24, fatture) un link alla fonte e alla citazione.
- **Monitoraggio normativo** ([docs/monitoraggio-normativo.md](docs/monitoraggio-normativo.md)): il job riscarica i documenti del registro, confronta l'impronta e segnala le fonti cambiate.
- Migrazione incrementale: prima le fonti dei set di regole 2025 e 2026, poi quelle della matrice di conformità e della normativa (comprese quelle verificate il 23-24/09/2026: Circ. INPS 62/2026, Spec. 1.9.1, guida AdE bollo, ris. 110/E/2019, art. 7-ter e 7-septies, TUIR art. 9, esempi delle ricevute SDI già in `packages/fatturapa/schemas/messaggi/`).
