  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-6 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile label="Emesso nell'anno" value={formatMoney(revenue)} hint="Imponibile + rivalsa, al netto delle note di credito" />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <CardHeader>
            ...
            <CardDescription>85.000 € di ricavi/compensi (L. 190/2014 c. 54). La soglia si misura sull&apos;incassato: qui è mostrato l&apos;emesso finché gli incassi non sono registrati.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            <div className="flex items-baseline justify-between text-sm">
              <span className="font-mono">{formatMoney(revenue)}</span>
              <span className="text-muted-foreground">{pct}%</span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-muted" role="meter" aria-valuemin={0} aria-valuemax={threshold} aria-valuenow={revenue} aria-label="Ricavi rispetto alla soglia">
              <div className="h-full rounded-full bg-primary" style={{ width: `${pct}%` }} />
            </div>
            {revenue > threshold && <p className="text-sm font-medium">⚠ Soglia superata: verifica le conseguenze per l&apos;anno successivo (c. 71).</p>}
          </CardContent>
        </Card>
