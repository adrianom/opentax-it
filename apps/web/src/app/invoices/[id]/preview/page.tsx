...
export default async function InvoicePreviewPage({ params }: PageProps<'/invoices/[id]/preview'>) {
  ...
  const [inv, me, terms, banks] = await Promise.all([
    fetchOrNull(() => api.invoice(id)),
    fetchOrNull(() => api.me()),
  ...
  const dueDate = chosenTerms ? new Date(new Date(inv.date).getTime() + chosenTerms.days * 86_400_000).toISOString().slice(0, 10) : undefined;

  const supplierName = profile ? (profile.businessName || `${profile.firstName} ${profile.lastName}`.trim()) : 'Fornitore';
    ...
    <div className="min-h-screen bg-muted/40 py-6 print:min-h-0 print:bg-white print:py-0">
      ...
      <div className="mx-auto mb-6 flex max-w-4xl items-center justify-between px-4 print:hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Anteprima {isDraft ? 'Bozza' : `${TYPE_LABELS[inv.type] ?? 'Fattura'} ${inv.number}`}</span>
        </div>
        <div className="flex items-center gap-2">
          <PrintButton />
      ...
      <main className="mx-auto max-w-4xl rounded-lg border bg-white p-8 text-neutral-900 shadow-sm print:m-0 print:max-w-none print:rounded-none print:border-none print:p-0 print:shadow-none md:p-12">
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-start">
          <div className="space-y-1">
            <h1 className="text-xl font-bold tracking-tight text-neutral-900">{supplierName}</h1>
            <p className="text-xs text-neutral-600 font-medium italic">Regime Fiscale: RF19 - Regime forfettario</p>
            {profile && (
              <div className="space-y-0.5 text-xs text-neutral-600">
                <p>P.IVA: {profile.vatNumber || '-'} · C.F.: {profile.fiscalCode || '-'}</p>
                <p>{profile.address}, {profile.postalCode} {profile.city} ({profile.province})</p>
                {profile.pecAddress && <p>PEC: {profile.pecAddress}</p>}
              </div>
            )}
          </div>
          ...
          <div className="min-w-[180px] rounded-md border border-neutral-200 bg-neutral-50/50 p-4 text-right">
            <span className="inline-block text-sm font-bold uppercase tracking-wider text-blue-900">
              {isDraft ? 'BOZZA PROFORMA' : (TYPE_LABELS[inv.type] ?? 'FATTURA')}
            </span>
        ...
        <div className="rounded-md border border-neutral-200 bg-neutral-50/40 p-4">
          ...
          <div className="mt-1 flex flex-col justify-between gap-2 sm:flex-row sm:items-baseline">
            <div className="space-y-0.5 text-xs">
              <p className="text-sm font-bold text-neutral-900">{custName}</p>
              <p className="text-neutral-600">{customer.address}, {customer.postalCode || ''} {customer.city} ({customer.province || customer.country})</p>
            </div>
            <div className="space-y-0.5 text-right text-xs text-neutral-600 sm:text-right">
              <p>P.IVA: {customer.vatNumber || '-'} · C.F.: {customer.fiscalCode || '-'}</p>
              <p>Codice Destinatario SDI: <span className="font-mono">{customer.recipientCode}</span>{customer.recipientPec ? ` · PEC: ${customer.recipientPec}` : ''}</p>
            </div>
          </div>
        </div>
          ...
          <Table>
            <TableHeader className="bg-neutral-100/70">
              <TableRow className="border-neutral-200 hover:bg-transparent">
                ...
                <TableHead className="text-right text-xs font-semibold text-neutral-700">IVA</TableHead>
            ...
            <TableBody>
              {inv.lines?.map((l) => (
                <TableRow key={l.lineNumber} className="border-neutral-100 hover:bg-neutral-50/50">
                  ...
                  <TableCell className="text-xs font-medium text-neutral-900">{l.description}</TableCell>
                  <TableCell className="text-right font-mono text-xs text-neutral-700">{Number(l.quantity)} {l.unit ?? ''}</TableCell>
                  <TableCell className="text-right font-mono text-xs text-neutral-700">{formatMoney(l.unitPrice, inv.currency)}</TableCell>
                  <TableCell className="text-right text-xs text-neutral-500">0% ({inv.vatNature.replace('_', '.')})</TableCell>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <div className="rounded-md border border-neutral-200 bg-neutral-50/30 p-4 text-xs">
            ...
            <div className="mt-2 space-y-1 text-neutral-700">
              <p><span className="text-neutral-500">Modalità:</span> {chosenTerms?.method === 'MP05' ? 'Bonifico bancario (MP05)' : (chosenTerms?.method ?? 'Bonifico')}</p>
              {dueDate && <p><span className="text-neutral-500">Scadenza:</span> <strong>{formatDate(dueDate)}</strong></p>}
              {chosenBank?.iban && <p><span className="text-neutral-500">IBAN:</span> <span className="font-mono font-semibold">{chosenBank.iban}</span></p>}
              {chosenBank?.bic && <p><span className="text-neutral-500">BIC/SWIFT:</span> <span className="font-mono">{chosenBank.bic}</span></p>}
              {chosenBank?.name && <p><span className="text-neutral-500">Istituto:</span> {chosenBank.name}</p>}
            </div>
          </div>
          ...
          <div className="flex flex-col justify-end space-y-1.5 text-xs">
            ...
            <div className="flex justify-between text-neutral-600">
              <span>IVA (0% - {inv.vatNature.replace('_', '.')}):</span>
        ...
        {inv.notes && inv.notes.length > 0 && (
          <div className="mt-8 rounded-md border border-neutral-200 bg-neutral-50/50 p-4">
            ...
            <ul className="mt-2 space-y-1 text-xs text-neutral-700">
              ...
              {inv.virtualStamp && (
                <li className="leading-relaxed">• Imposta di bollo assolta in modo virtuale ai sensi del D.M. 17 giugno 2014.</li>
              )}
            </ul>
          </div>
        )}
        ...
        <div className="mt-12 border-t border-neutral-200 pt-4 text-center text-[10px] text-neutral-400">
          <p>Copia di cortesia priva di valenza fiscale ai sensi dell&apos;art. 21 DPR 633/72. L&apos;originale della fattura elettronica è trasmesso e conservato digitalmente tramite il Sistema di Interscambio (SDI).</p>
        </div>
      </main>
    </div>
  );
