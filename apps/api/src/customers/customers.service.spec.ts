...
import type { CreateCustomerDto } from './customers.dto.js';
...
describe('CustomersService kind and country', () => {
  it('refuses a United Kingdom or Northern Ireland customer created as EU', async () => {
    ...
    for (const countryCode of ['GB', 'XI']) {
      await expect(svc.create('t1', { ...base, kind: 'EU', countryCode } as CreateCustomerDto)).rejects.toThrow(BadRequestException);
    }
  });

  it('accepts them as NON_EU, with XXXXXXX and CAP 00000', async () => {
    const { svc } = service();
    const c = await svc.create('t1', { ...base, kind: 'NON_EU', countryCode: 'GB' } as CreateCustomerDto);
  ...
  it('refuses an EU member state created as NON_EU and accepts it as EU', async () => {
    const { svc } = service();
    await expect(svc.create('t1', { ...base, kind: 'NON_EU', countryCode: 'DE' } as CreateCustomerDto)).rejects.toThrow("è uno Stato membro dell'UE");
    await expect(svc.create('t1', { ...base, kind: 'EU', countryCode: 'DE' } as CreateCustomerDto)).resolves.toMatchObject({ kind: 'EU' });
  });
});
